      ...
      <div className="w-full max-w-[460px] mx-auto px-4">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex space-x-4 text-sm font-medium">
            <button 
              className={activeSwapType === 'swap' ? 'text-fuel-green' : 'text-gray-400 hover:text-gray-300'}
              onClick={() => setActiveSwapType('swap')}
            >
              Swap
            </button>
            <button 
              className={activeSwapType === 'limit' ? 'text-fuel-green' : 'text-gray-400 hover:text-gray-300'}
          ...
          <div className="relative">
            <button 
              className="p-2 hover:bg-fuel-dark-700 rounded-full transition-colors"
                  ...
                  {activeSwapType === 'swap' ? (
                    <>
                      ...
                      <div className="space-y-2 mb-4">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-1">
                            <span className="text-sm text-gray-400">MEV-protected slippage</span>
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <div className="relative flex-1">
                            <input
                              ...
                              value={slippageTolerance === 'auto' ? slippageDisplay : slippageTolerance}
                              onChange={(e) => {
                      ...
                      <div className="space-y-2 mb-4">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-1">
                            <span className="text-sm text-gray-400">Swap deadline</span>
                            <Info className="w-4 h-4 text-gray-500" />
                          </div>
                        </div>
                        <div className="relative">
                    ...
                    <>
                      <h3 className="text-sm font-medium mb-4">Interface Settings</h3>
                      
                      {/* Custom Recipient Toggle */}
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center space-x-1">
                          <span className="text-sm text-gray-400">Custom Recipient</span>
                          <Info className="w-4 h-4 text-gray-500" />
                        </div>
                        <button 
                          className={`w-10 h-6 rounded-full transition-colors relative ${
                            customRecipient ? 'bg-fuel-green' : 'bg-fuel-dark-700'
                          }`}
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-1">
                          <span className="text-sm text-gray-400">Enable Partial Executions</span>
                          <Info className="w-4 h-4 text-gray-500" />
                        </div>
                        <button 
                          className={`w-10 h-6 rounded-full transition-colors relative ${
                            enablePartialExecutions ? 'bg-fuel-green' : 'bg-fuel-dark-700'
                          }`}
                          onClick={() => setEnablePartialExecutions(!enablePartialExecutions)}
                        >
                          <div className={`absolute w-4 h-4 rounded-full bg-white top-1 transition-all ${
        ...
        {activeSwapType === 'swap' ? (
          <div className="bg-fuel-dark-800 rounded-xl p-4 shadow-lg">
            {/* From Token */}
            <div className="space-y-3">
              ...
              <div className="flex items-center space-x-2 bg-fuel-dark-700 p-3 rounded-lg">
                <button
                  className="flex items-center space-x-2 px-2 py-1.5 rounded-lg hover:bg-fuel-dark-600 min-w-[120px]"
                  onClick={() => setIsFromTokenOpen(!isFromTokenOpen)}
                >
                  <div className={`w-6 h-6 rounded-full ${fromToken.iconBg} flex items-center justify-center`}>
                ...
                <input
                  type="text"
                  className="flex-1 bg-transparent text-2xl font-medium focus:outline-none text-right"
                  placeholder="0.00"
                  value={fromAmount}
                  onChange={(e) => setFromAmount(e.target.value)}
                />
              </div>
              <div className="text-right text-sm text-gray-400">≈ ${fromToken.usdValue}</div>
            </div>

            {/* Swap Direction Button */}
            <div className="flex justify-center my-2">
              <button 
                className="p-2 rounded-lg bg-fuel-dark-700 hover:bg-fuel-dark-600 transition-colors"
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center space-x-2 bg-fuel-dark-700 p-3 rounded-lg">
                ...
                >
                  <div className={`w-6 h-6 rounded-full ${toToken.iconBg} flex items-center justify-center`}>
              ...
              <div className="text-right text-sm text-gray-400">≈ ${toToken.usdValue}</div>
            </div>

            {/* Additional Info */}
            <div className="mt-4 space-y-2">
              <div className="flex items-center justify-between text-sm p-3 rounded-lg bg-fuel-dark-700">
                <div className="flex items-center space-x-1">
                  <span className="text-gray-400">1 ETH = 78,079.0602 FUEL</span>
                  <Info className="w-4 h-4 text-gray-500" />
                </div>
                <span className="text-gray-400">(≈ $3,137.58)</span>
              </div>

              <div className="space-y-2 p-3 rounded-lg bg-fuel-dark-700">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span>{slippageTolerance === 'auto' ? 'Auto' : `${slippageDisplay}%`}</span>
                </div>
                <div className="flex justify-between text-sm">
          ...
          <div className="bg-fuel-dark-800 rounded-xl p-4 shadow-lg">
            ...
            <div className="space-y-3">
              <div className="flex justify-between text-xs">
                ...
                <div className="flex items-center space-x-2">
                  <span className="text-gray-400">Balance: {fromToken.balance} {fromToken.symbol}</span>
                  <button className="text-xs text-fuel-green">MAX</button>
                </div>
              </div>
              <div className="flex items-center space-x-2 bg-fuel-dark-700 p-3 rounded-lg">
                <button
            ...
            <div className="grid grid-cols-3 gap-2 mt-4">
              <div className="col-span-2">
                <div className="text-xs text-gray-400 mb-2">Limit price <span className="text-green-400">(+0.1%)</span></div>
                <div className="flex items-center bg-fuel-dark-700 p-3 rounded-lg">
                  <input
                    type="text"
                  ...
                  />
                  <button className="text-sm text-gray-400 hover:text-gray-300 px-2">
                    {fromToken.symbol}
                  </button>
                </div>
              </div>
              <div>
                <div className="text-xs text-gray-400 mb-2">Expiry</div>
                <div className="relative">
                  <button
                    className="w-full bg-fuel-dark-700 p-3 rounded-lg text-left flex items-center justify-between"
            ...
            {/* Review Button */}
            <div className="mt-4">
              <button className="w-full py-3 bg-fuel-green text-fuel-dark-900 rounded-lg font-medium hover:bg-opacity-90 transition-colors">
                Place limit order
              </button>
            </div>
          </div>
